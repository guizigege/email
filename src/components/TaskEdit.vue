    ...
    <div style="padding:20px 20px 0 20px;">
      ...
      <div style="height:20px;line-height: 20px;padding-left: 20px;">
        <el-breadcrumb separator-class="el-icon-arrow-right" >
          ...
          <el-breadcrumb-item :to="{ path: '/home/task/add' }">编辑任务</el-breadcrumb-item>
          <el-breadcrumb-item></el-breadcrumb-item>
        </el-breadcrumb>
      </div>
      <!--新建任务-->
      <div class="setParamDialog" style="padding:20px 0;box-sizing: border-box">
        <div style="background-color: steelblue;color:#fff;height:30px;line-height: 30px;padding-left: 20px;font-size: 14px;font-weight: bold;">编辑任务号：{{addTask.id}}</div>
          ...
          <div class="task-item">
            ...
            <div style="width:800px;margin-top: 10px;">
              <el-checkbox-group v-model="addTask.computer_list">
                <el-checkbox v-for="(item,index) in computerList" :key="index" :label="item.id" :value="item.id" name="type" class="email-task" :disabled="(item.task_id!=-1)&&!(item.task_id==addTask.id)||(item.machine_state===1)">{{item.machine_ip}}|{{item.machine_name}}</el-checkbox>
              </el-checkbox-group>
            </div>
          </div>
          <div class="task-item">
...
<script>
  export default {
    ...
    methods: {
      //读取单个任务的配置信息
      get_task(){
        this.$get('/task/info',{id:parseInt(this.$route.params.id)}).then((res)=>{
          this.addTask = res.data;
          this.timeStart = this.addTask.begin_date+" "+this.addTask.begin_time;
          this.timeEnd = this.addTask.end_date+" "+this.addTask.end_time;
        });
